...
function FeaturePanel({ tone }: { tone: "blue" | "amber" | "green" | "purple" }) {
  return (
    <VisualPlaceholder tone={tone} className="h-[250px] rounded-[10px] border-[#d8e4ef]">
...
function HeroDashboardPanel() {
  return (
    <VisualPlaceholder tone="slate" className="mx-auto h-[260px] max-w-[900px] rounded-[10px] border-[#d8dee6]">
...
export default function FeaturesPage() {
  ...
  return (
    <>
      <section className="msp-hero-soft msp-hero-rings py-14 md:py-16">
        <Container size="content" className="text-center">
          <h1 className="mx-auto max-w-[840px] text-[54px] font-bold leading-[1.1] tracking-[-0.03em] text-msp-ink">
            {hero.title}
          </h1>
          <p className="mx-auto mt-4 max-w-[880px] text-[15px] leading-7 text-msp-ink">{hero.subtitle}</p>
            ...
            {sections.map((section) => {
              ...
              const text = (
                <div>
                  ...
                  <p className="mt-3 text-[14px] leading-7 text-msp-ink">{section.body}</p>
      ...
      <section className="bg-white pb-16 pt-8">
        <Container size="content">
          <div className="msp-hero-soft grid items-center gap-6 rounded-[10px] border border-[#d7e4f2] p-7 md:grid-cols-[1fr_220px]">
            <div>
              <p className="max-w-[620px] text-[15px] leading-7 text-msp-ink">
                <span className="font-bold text-msp-blue">With almost 90% of SMBs</span> {closing.body}
              </p>
              <Button href={closing.cta.href} size="sm" className="mt-4 h-[42px] px-6 text-[12px]">
                {closing.cta.label}
              </Button>
            </div>
            <VisualPlaceholder tone="slate" className="h-[126px] rounded-[8px] border-[#cfdae7]">
